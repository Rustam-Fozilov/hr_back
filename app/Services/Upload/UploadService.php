...
class UploadService
{
    ...
    {
        if (!is_null($id)) $this->file = Upload::find($id);

        if (!is_null($url)) $this->file = Upload::where('url', $url)->first();
    }

    public function set($url): static
    {
        $this->file = Upload::where('url', $url)->first();
        return $this;
    }
    ...
    {
        return (bool)Upload::where('path', $path)->first();
    }
